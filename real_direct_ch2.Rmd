...
as.numeric(as.character(manhattan.sales$GROSS.SQUARE.FEET))
as.numeric(as.character(manhattan.sales$LAND.SQUARE.FEET))

# subset of data: remove columns because all borough values == 1, ease.ment column are all NAs and we have made a new column (SALE.PRICE.N) for sale price as integers.
manhattan.sales <- manhattan.sales %>% select(-BOROUGH, -EASE.MENT,-SALE.PRICE)


# All neighbourhoods and zip codes   
...
plot(manhattan.sales$LAND.SQUARE.FEET, manhattan.sales$SALE.PRICE.N)
...
ggplot(data = manhattan.sales, aes(manhattan.sales$NEIGHBORHOOD, log(manhattan.sales$SALE.PRICE.N)))+
...
ggplot(manhattan.sales, aes(x=newdate, y=log(SALE.PRICE.N))) + 
  ...
  scale_colour_manual(labels = c("month means","error bars"),
                      values = c("red", "red"))+
  labs(x = "Date", y = 'log Sale Prices', colour = " ", title = "Time Series of Log Sale Prices")+
  theme_bw()+
  theme(plot.title = element_text(size = 14, face = "bold", color = "black", hjust = 0.5))

#TODO weekly trends

#TODO quartley trends
